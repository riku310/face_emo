...
while(True):
    for i, capture in enumerate( captures ):
        ...
        cascadepath = '/Users/riku/test/haarcascade_frontalface_default.xml'
        front_face_cascade = cv2.CascadeClassifier(cascadepath)
        if front_face_cascade.empty():
            print("Error: Front face cascade not loaded.")
            exit()
        faces = front_face_cascade.detectMultiScale(gray, scaleFactor=1.1, minNeighbors=5, minSize=(30, 30))
        '''
        ...
        for(x,y,w,h) in faces:  
            ...
            # PILのImageモジュールを使用して画像を配列に変換
            img_pil = Image.fromarray(face_cut)
            img_array = np.array(img_pil)

            # ニューラルネットワークに入力するために配列の次元を変更し、0から1の範囲に正規化
            pImg = np.expand_dims(img_array, axis=0) / 255
